...
test(`${BLOCK_TYPE}: registered in block registry`, () => {
  ...
  assert.equal(meta.displayName, "Carousel")
...
test(`${BLOCK_TYPE}: schema rejects empty items array`, () => {
  const result = validateBlockProps(BLOCK_TYPE, { items: [] })
  assert.equal(result.success, false)
})

test(`${BLOCK_TYPE}: schema rejects item with empty imageUrl`, () => {
  const result = validateBlockProps(BLOCK_TYPE, {
    items: [{ imageUrl: "", heading: "Test" }],
  })
  assert.equal(result.success, false)
})

test(`${BLOCK_TYPE}: schema accepts item with only imageUrl`, () => {
  const result = validateBlockProps(BLOCK_TYPE, {
    items: [{ imageUrl: "/img.jpg" }],
  })
  assert.equal(result.success, true)
})

test(`${BLOCK_TYPE}: schema accepts optional autoplay and interval`, () => {
  const result = validateBlockProps(BLOCK_TYPE, {
    items: [{ imageUrl: "/img.jpg" }],
    autoplay: "true",
    interval: 3000,
