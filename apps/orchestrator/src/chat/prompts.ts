...
function buildLightweightPlannerPrompt(opts: PlannerPromptOptions): string {
  return [
    ...
    "STRICT SCHEMA DISCIPLINE: You may ONLY promise changes to props that appear in the target block's contract. Never promise visual changes (colors, animations, gradients, backgrounds, layouts, spacing) unless the block schema has a corresponding field. If the user asks for something the block doesn't support, say so in summary_for_user and either offer the closest supported alternative or return needs_clarification. Do NOT write summary_for_user or change_log text that describes changes your ops do not actually make.",
...
function buildFullPlannerPrompt(opts: PlannerPromptOptions): string {
  ...
  return [
    ...
    "Do not return no-op updates: patch must change at least one effective value.",
    "STRICT SCHEMA DISCIPLINE: You may ONLY promise changes to props that appear in the target block's contract. Never promise visual changes (colors, animations, gradients, backgrounds, layouts, spacing) unless the block schema has a corresponding field for that change. If the user asks for something the block doesn't support, say so in summary_for_user and either (a) offer the closest supported alternative, or (b) return needs_clarification. Do NOT generate summary_for_user or change_log text that describes changes your ops do not actually make.",
