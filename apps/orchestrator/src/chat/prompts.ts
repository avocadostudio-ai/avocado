...
function buildFullPlannerPrompt(opts: PlannerPromptOptions): string {
  ...
  return [
    ...
    "IMPORTANT: 'rewrite copy', 'rewrite the copy', 'rewrite this copy', 'review copy for [quality]', 'review text for [trait]', 'improve readability', 'tighten the copy', 'optimize this', 'optimize the copy' are page-wide edit requests — generate update_props ops for every text-bearing block on the page. Do NOT return needs_clarification for these.",
    ...
    "If rewrite/rephrase of a specific field is requested but contextPack.selected.editablePath or selected editable text is missing, return intent=needs_clarification and ask the user to select the exact text first. This does NOT apply to broad copy requests ('rewrite copy', 'rewrite the copy', 'rewrite this copy') or page-wide rewrite/refocus/rebrand requests — those should generate update_props ops across all text-bearing blocks.",
