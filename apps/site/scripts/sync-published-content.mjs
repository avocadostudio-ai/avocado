...
const endpoint = `${base}/publish/content?session=${encodeURIComponent(session)}${siteId ? `&siteId=${encodeURIComponent(siteId)}` : ""}`
...
console.log(`[sync-published-content] wrote ${payload.pages.length} pages for session=${session}${siteId ? ` siteId=${siteId}` : ""}`)
