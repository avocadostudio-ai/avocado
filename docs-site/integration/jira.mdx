...
**Execute** — The agent runs with full tool access, including the complete comment history so any clarifications posted as replies reach the agent. It reads the current site state, applies ops, and transitions the ticket to the preview status (`In Review` by default). It posts a comment listing every change made and links to the live preview.
