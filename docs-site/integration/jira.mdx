...
**Review** — The agent reads the ticket (summary, description, attachments, comment history) and posts one of two comments:
- A plan: *"Here's what I'll do — let me know when to proceed."*
- Questions: *"I need more info on X before I can start."*

No site changes are made during review.

**Execute** — The agent runs with full tool access: it reads the current site state, applies ops, and transitions the ticket to the preview status (`In Review` by default). It posts a comment listing every change made and links to the live preview.
...
The agent tracks its own Jira account ID (`JIRA_AGENT_ACCOUNT_ID`). Comments and status changes authored by that account are always skipped. Without this, the agent's own comment on a ticket would re-trigger processing.
...
| Comment (any status) | Any | `@site-editor` / `@website-agent` mention | **review** |
