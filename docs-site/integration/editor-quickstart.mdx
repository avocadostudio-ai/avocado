...
MVP requirement:
- Expose `GET /api/editor/blocks` so the editor/orchestrator knows which components are safely editable.
...
The `@ai-site-editor/site-sdk` package provides handler factories for all required API routes — see [nextjs-mvp-embedded](/integration/nextjs-mvp-embedded).
