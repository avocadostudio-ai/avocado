...
description: "Quick guide to integrating the editor with an existing Next.js site in embedded draft mode."
---

# Editor Integration Quickstart (Embedded Draft Mode)

Use this when integrating the editor with any existing Next.js site. For background on core concepts, see [Core Concepts](/concepts).
...
`createEditorApiHandler()` automatically builds the manifest from the 20 built-in block types. The editor and AI planner read this manifest to understand what blocks exist, what fields they have, and what values are valid.
