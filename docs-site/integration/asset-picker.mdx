...
Avocado Studio has two entry points for images: the **Asset Picker modal** in
...
```
Editor image field click
        │
        ▼
┌───────────────────────────────────┐
│       Asset Picker modal          │
│  Generate │ Drive │ Unsplash │    │
│  CMS      │ Upload                │
└──────┬────────────────────────────┘
       │
       ▼                            ┌────────────────────────┐
┌────────────┐  ┌────────────┐      │ Chat: "generate a hero │
│  OpenAI    │  │  Google    │◄─────│  image of…"            │
│  DALL-E    │  │  Gemini    │      └──────────┬─────────────┘
│ gpt-image-1│  │ 2.5 Flash  │                 │
└────────────┘  │  Image     │                 ▼
                │ (nano-     │      ┌──────────────────────┐
                │  banana)   │      │  image.generate tool │
                └────────────┘      └──────────────────────┘
```
...
- [Custom Blocks › Image fields](/integration/custom-blocks) — how to declare
  image fields on your own blocks so the Asset Picker opens for them.
