...
description: "Deploy Avocado Studio site and Content Studio to Netlify."
---
...
|-----------|----------------|-------|
| **Site** (`apps/site`) | Netlify Sites | Next.js via `@netlify/plugin-nextjs` |
| **Content Studio** (`apps/editor`) | Netlify Sites | Static Vite build |
...
The orchestrator requires a long-running process. Deploy on Render, Fly.io, Railway, or any container platform. See the [Vercel deployment guide](/operations/vercel-deployment#orchestrator-on-render) for Render-specific instructions.
...
| `DRAFT_MODE_SECRET` | Shared secret (must match Content Studio's `VITE_SITE_DRAFT_SECRET`) |
...
Contributions welcome! See the [contributing guide](https://github.com/avocadostudio-ai/avocado/blob/main/CONTRIBUTING.md).

## Troubleshooting

See the [production troubleshooting](/operations/vercel-deployment#production-troubleshooting) section — the same postMessage, CORS, and draft mode issues apply regardless of hosting platform.
