---
title: "AI content ops for any Next.js website"
sidebarTitle: "Welcome"
description: "Free and open source. Agentic-first. Bring AI-native editing to any Next.js website — self-hostable, BYO LLM keys, composable with the CMS, DAM, and stack you already have."
---

{/* Intro video — YouTube embed (nocookie, no related videos, modest branding). */}
<div className="relative w-full rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800" style={{ aspectRatio: "16 / 9" }}>
  <iframe
    ...
    src="https://www.youtube-nocookie.com/embed/QPQOZIJlC98?rel=0&modestbranding=1&playsinline=1"
    title="Avocado Studio demo"
    frameBorder="0"
...
- **Sitecore** — [SitecoreAI](https://doc.sitecore.com/sai/en/users/sitecoreai/sitecoreai.html) as the AI-first flagship, [Sitecore Stream](https://www.sitecore.com/platform/sitecore-stream) as the AI engine, and the [Agentic Studio](https://doc.sitecore.com/sai/en/users/sitecoreai/working-with-agentic-studio.html)
...
This **BYO-stack** approach is the difference between Avocado and a traditional all-in-one DXP. Suites like Adobe Experience Manager and Sitecore are designed as integrated end-to-end platforms — content, assets, personalization, delivery, and now agentic AI all sold together as one product, with the consistency and depth that shape brings. Avocado takes the opposite shape on purpose: it stays narrow to the editing and AI-orchestration layer and lets you choose every system underneath it. Both shapes are valid; Avocado exists for teams who specifically want the composable one.

## What is Avocado Studio?

Avocado Studio is an open, agentic-first editing layer **for websites** — purpose-built for **Next.js** today, with a Site SDK contract that other frameworks can adopt. Describe content changes in natural language or use the visual drag-and-drop editor — the system generates schema-validated operations, lets you review the plan, and applies approved changes to draft content with live preview, undo/redo, and publishing controls.
...
<CardGroup cols={2}>
  ...
  <Card title="Onboard a site — with the AI agent" icon="robot" href="/sites/site-agent">
  ...
  <Card title="Onboard a site — code it yourself" icon="screwdriver-wrench" href="/sites/manual">
    The manual path. Wire `@ai-site-editor/site-sdk` into your existing Next.js 15 project by hand (~30 min) or hand the work to your own coding agent (Codex / Claude Code / Cursor).
  </Card>
  <Card title="Visual editor" icon="hand-pointer" href="/integration/puck-mode">
    Enable the drag-and-drop editor for a site (alternative to the chat editor).
  </Card>
  <Card title="Drive Avocado from Claude (MCP)" icon="plug" href="/integration/mcp-server">
    Plug into Claude Desktop, Claude Code, or any Model Context Protocol host. 40 tools for reading, editing, publishing, and image generation — all the operations the web editor uses.
  </Card>
  <Card title="Edit from Jira tickets" icon="ticket" href="/integration/jira">
    Move a ticket to Review and Avocado plans the change, applies edits to draft, posts a preview link, and waits for your approval comment before publishing.
  </Card>
</CardGroup>
...
- **MCP server (drive from Claude Desktop / Code)** — Avocado ships a [Model Context Protocol server](/integration/mcp-server) with 40 tools (read, edit, publish, generate images, manage sites). Plug it into any MCP host and your AI assistant edits your site through the same operations the web editor uses.
- **Jira-driven editing** — Move a ticket through Review → Execute → Preview and Avocado plans the change, applies edits, posts a preview link in the ticket, and waits for your approval comment before publishing. See [Jira Integration](/integration/jira).
...
4. [Add a site](/sites) — bring an existing website into Avocado Studio (Onboarding agent or by hand)
