...
description: "Open, agentic, composable content operations. A modern AI-native content layer for everyone who wants the kind of agentic editing experience the largest enterprise DXPs (Adobe AEM, Sitecore, Contentstack, Optimizely…) are racing to ship — without their license fees, agency contracts, or multi-month implementations."
---
...
- **Open, self-hostable, "free"** — no per-seat license fees, no annual contracts, no minimum spend, no procurement cycles. Run the whole stack on a small VPS or your existing container host. Bring your own LLM keys (or use the demo mode). The total cost of running Avocado is what you pay your cloud provider, plus what you spend on LLM tokens.
...
<Note>
**Honest scope today.** Avocado Studio is in early/active development. The Next.js integration path is rock-solid and tested in production; other frameworks are first-mover territory. The orchestrator is distributed as a Docker image for self-hosting — the source is not public yet, but the binary is free to run on your own infrastructure (see [Docker Deployment](/operations/docker-deployment)). The site templates, editor app, and SDK are all source-distributed and live in the public repo. If you're a founder, an agency, an in-house marketing team, an in-house developer, or just someone who wants in early — welcome. File issues, contribute back, help shape what this becomes.
</Note>
...
- **The [Site SDK](/integration/nextjs-integration)** — connects your CMS / database / file system as the source of truth for pages and blocks. The SDK ships with adapters for Next.js out of the box, plus working examples for Strapi and Contentful. Sanity, Notion, Markdown, JSON files, or your own database all work via the same `getPage` / `getSlugs` / `getSiteConfig` callbacks.
...
- **AI image generation built in** — DALL-E (`gpt-image-1`), Google Gemini (`gemini-2.5-flash-image`, "nano-banana"), and Unsplash search ship out of the box. See [Asset Picker & AI Images](/features/asset-picker).
