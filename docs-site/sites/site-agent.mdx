...
This is *only* a constraint for the Site Assistant. The editor's per-edit chat (the AI you talk to *after* a site exists) already supports OpenAI and Anthropic — see [model selection](/integration/editor-quickstart). Multi-provider support for the Site Assistant is on the roadmap but not implemented today; if it matters to you, the **[Bring Your Own Coding Agent](/sites/coding-agent)** path lets you use any agent (Codex, Cursor, Aider, …) regardless of which model it runs on.
</Note>
