...
description: "Two ways to bring a website into Avocado Studio: let our Site Assistant do it for you, or code it yourself (by hand or with your own coding agent). Currently supports Next.js."
---
...
<CardGroup cols={2}>
  <Card title="Agentic — Site Assistant" icon="robot" href="/sites/site-agent">
...
| If you… | Use this |
|---|---|
| Want the fastest possible result | **Site Assistant** |
| Have a **public website URL** (any tech) and want AI to scrape it and rebuild it as a Next.js site | **Site Assistant → Migrate** |
| Have an **existing Next.js project on GitHub** and want AI to clone it and wire the SDK in | **Site Assistant → Integrate** |
| Have just an idea and want AI to scaffold a fresh Next.js site for you | **Site Assistant → Create** |
...
## Use our Site Assistant

The Site Assistant is an AI agent in the editor's *Sites* page. Pick one of three modes from your first message:
...
**Heads up: Claude only.** The Site Assistant runs exclusively on Anthropic Claude (via your Claude subscription or an `ANTHROPIC_API_KEY`). An OpenAI key alone won't work for *this* path — though the editor's per-edit chat supports OpenAI or Anthropic once a site exists. If you need a different model, use the **[Bring Your Own Coding Agent](/sites/coding-agent)** path with any agent you like.

→ Full reference: **[Site Assistant](/sites/site-agent)** (modes, prerequisites, CLI vs SDK billing, troubleshooting)
