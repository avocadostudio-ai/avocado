...
- Node 20+ and pnpm
- A Next.js 15 project (App Router) — or any framework that can implement the [Site Provider SPI](/site-provider-spi)
- The Avocado orchestrator running locally or remotely (`pnpm dev:orchestrator`)
- One of: `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or demo mode (no key) for testing
...
<Steps>
  ...
  <Step title="Wire the SDK into your Next.js site">
    Follow **[Next.js Embedded MVP](/integration/nextjs-mvp-embedded)**. This is the meat — five contract endpoints (`GET /api/editor/blocks`, `GET /api/draft`, `GET /api/draft/disable`, page data loader changes, and the iframe URL pattern). The `createBlocksHandler()` helper from the SDK does most of the work.
...
|---|---|---|
| Register the site with the orchestrator's site registry | Yes (`register_site` MCP tool) | You need to call it yourself or use the editor's "Add Site" UI |
