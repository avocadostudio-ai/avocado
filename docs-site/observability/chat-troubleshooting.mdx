...
This playbook is for investigating prompt failures, wrong operations, and regressions in the site editor assistant. For background on the editing pipeline, see [How It Works](/how-it-works). For the full telemetry event reference, see [Chat Telemetry Events](/observability/chat-telemetry-events).
...
1. Open Content Studio settings (gear icon).
2. Enable `Debug mode`.
