...
| **Agent tool (`generate_variations`)** | Onboarding agent / agent SDK loop | `apps/orchestrator/src/agent/agent-tools.ts` (lines ~699-768) | Claude authors the variants *inside* its reasoning loop — no separate LLM call. Fast, tool-use-native, no image variation |
...
When the Onboarding agent is active, Claude calls the `generate_variations` tool
