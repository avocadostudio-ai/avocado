...
2. If `deriveVariationImageIntent` classifies the message as wanting AI-
   generated images (`imageIntent.provider === "llm"`), the pipeline calls
   `generateVariationImageWithOpenAI` with a per-variant prompt built from
   the block type, heading, and subheading.
