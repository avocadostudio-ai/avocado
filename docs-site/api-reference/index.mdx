...
This tab documents the **Orchestrator HTTP API** — the brain that runs editor sessions, calls the LLMs, and serves draft state to integrated sites. The same API is what the editor app, the `avocado-register` CLI, the Site Assistant, and any [Bring Your Own Coding Agent](/sites/coding-agent) workflow all talk to.
...
| **Sites Agent** | Site onboarding agent (migrate / integrate / create modes) — `POST /sites-agent/start`, `GET /sites-agent/stream`, `POST /sites-agent/respond`, `POST /sites-agent/cancel` | Editor's Site Assistant FAB |
...
| **Media** | Image upload, generation, search — `POST /image/upload`, `POST /image/generate`, `POST /image/generate/chat`, `POST /image/interpret`, `GET /unsplash/search`, `GET /generated-images/:fileName` | Editor's Asset Picker |
