...
Other events emitted on the same stream include `field_draft`, `summary_token`,
...
| **Deferred image resolution** | Text and structural changes apply immediately. Image lookups and generation (Unsplash, OpenAI `gpt-image-1`, and Google Gemini `gemini-2.5-flash-image`) resolve in the background. See [Asset Picker & AI Images](/integration/asset-picker). |
