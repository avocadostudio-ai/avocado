...
Three targets ship in the box, registered at module load (in order) from `apps/orchestrator/src/publish/publish-target-registry.ts`:
...
|----------|---------|-------------|
| `PUBLISH_MODE` | `git` | Legacy selection: `git` or `deploy_hook` (used when no `canHandle` match and no `PUBLISH_TARGET` override) |
| `PUBLISH_TARGET` | _(none)_ | Force a specific target by name (overrides `canHandle` and `PUBLISH_MODE`) |
...
| `VERCEL_DEPLOY_HOOK_URL` | _(none)_ | Vercel deploy hook URL (for `deploy-hook` target) |
